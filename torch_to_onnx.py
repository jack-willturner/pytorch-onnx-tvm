...
for block in resnet50.children():
    ...
    if isinstance(block, nn.Conv2d):
        ...
        torch.onnx.export(block, dummy_input, ('resnet/resnet50_' + str(i) + '.onnx'))
        i += 1
        ...
        for layer in block:
            ...
            df.append([layer.conv1.in_channels, layer.conv1.out_channels, layer.conv1.kernel_size,
                            layer.conv1.stride, layer.conv1.padding, layer.conv1.bias, layer.conv1_input_size[2],
                            layer.conv1_input_size[3], ops, params])
            torch.onnx.export(layer.conv1, dummy_input, ('resnet/resnet50_' + str(i) + '.onnx'))
            i += 1
            ...
            df.append([layer.conv2.in_channels, layer.conv2.out_channels, layer.conv2.kernel_size,
                            layer.conv2.stride, layer.conv2.padding, layer.conv2.bias, layer.conv2_input_size[2],
                            layer.conv2_input_size[3], ops, params])
            torch.onnx.export(layer.conv2, dummy_input, ('resnet/resnet50_' + str(i) + '.onnx'))
            i += 1
            ...
            df.append([layer.conv3.in_channels, layer.conv3.out_channels, layer.conv3.kernel_size,
                            layer.conv3.stride, layer.conv3.padding, layer.conv3.bias, layer.conv3_input_size[2],
                            layer.conv3_input_size[3], ops, params])
            torch.onnx.export(layer.conv3,dummy_input , ('resnet/resnet50_' + str(i) + '.onnx'))
            i += 1
...
df = pd.DataFrame(df, columns=['in_channels', 'out_channels', 'kernel_size', 'stride', 'padding', 'bias', 'input_spatial_x', 'input_spatial_y', 'ops', 'params'])
